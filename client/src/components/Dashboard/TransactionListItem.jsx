...
const TransactionListItem = ({
  ...
  return (
    <div className="bg-gray-50 dark:bg-gray-800 rounded-lg pt-3 pb-3">
      <div className="flex justify-between items-center gap-3" onClick={() => setIsExpanded(!isExpanded)}>
        ...
        <div className="flex items-center">
          <span className={`font-semibold text-base whitespace-nowrap ${transaction.type === 'credit' ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}>
      ...
      {isExpanded && (
        <div className="mt-3 border-t border-gray-200 dark:border-gray-700 pt-3 space-y-1">
          <div>
            <label className="block text-[0.65rem] font-medium text-gray-500 dark:text-gray-400">Descrição</label>
          ...
          <div>
            <label className="block text-[0.65rem] font-medium text-gray-500 dark:text-gray-400">Valor (R$)</label>
            <EditableTableCell
          ...
          <div className="relative">
            <label className="block text-[0.65rem] font-medium text-gray-500 dark:text-gray-400 mb-1">Categoria</label>
            <div className="flex items-center justify-between gap-2">
              <div
                onClick={() => toggleCategoryEditor(transaction.id)}
                className="flex-grow flex items-center justify-between cursor-pointer p-1 pl-2 border rounded-lg dark:bg-gray-700 dark:border-gray-600 dark:text-white hover:bg-gray-100 dark:hover:bg-gray-600 transition-colors"
              >
                <span className="text-sm sm:text-base">{transaction.category?.name || 'Selecione uma categoria...'}</span>
